...
function buildUrlFromTemplate(template, params) {
  ...
  // Simple approach: replace $AUTH_BASE_URL with the actual value
  if (process.env.AUTH_BASE_URL) {
    resolvedTemplate = resolvedTemplate.replace(/\$AUTH_BASE_URL/g, process.env.AUTH_BASE_URL);
  }
  
  // Handle ${VAR} syntax
  resolvedTemplate = resolvedTemplate.replace(/\$\{([^}]+)\}/g, (match, varName) => {
...
async function getPassengerDetails(id, token) {
  try {
    ...
    logger.info('[external.passenger.get] request', { 
      id: String(id), 
      template: tpl,
      resolvedUrl: url,
...
async function getDriverDetails(id, token) {
  try {
    ...
    const url = buildUrlFromTemplate(tpl, { id });
    logger.info('[external.driver.get] request', { 
      id: String(id), 
      template: tpl,
      resolvedUrl: url,
      authBase: getAuthBase()
